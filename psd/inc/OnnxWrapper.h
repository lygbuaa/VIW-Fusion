...
private:
    ...
    ORT_S_t g_dmpr_s_;
    OutputRaw_t g_det_;
    std::vector<float> g_input_vec_fp32_;
...
public:
    ...
    bool load_dmpr_model(const std::string& model_path){
        return load_model(model_path, g_dmpr_s_);
    }

    /* img should be rgb format */
    bool run_dmpr_model(const cv::Mat& img, OutputRaw_t& result){
        /* preprocess */
        ...
        HANG_STOPWATCH();
        /* prepare input data */
        std::vector<const char*>& input_node_names = g_dmpr_s_.input_node_names;
        std::vector<std::vector<int64_t>>& input_node_dims = g_dmpr_s_.input_node_dims;
        std::vector<ONNXTensorElementDataType>& input_types = g_dmpr_s_.input_types;
        std::vector<OrtValue*>& input_tensors = g_dmpr_s_.input_tensors;

        std::vector<const char*>& output_node_names = g_dmpr_s_.output_node_names;
        std::vector<std::vector<int64_t>>& output_node_dims = g_dmpr_s_.output_node_dims;
        std::vector<ONNXTensorElementDataType>& output_types = g_dmpr_s_.output_types;
        std::vector<OrtValue*>& output_tensors = g_dmpr_s_.output_tensors;
        ...
        CheckStatus(g_ort_->CreateTensorWithDataAsOrtValue(
                        memory_info, reinterpret_cast<void*>(g_input_vec_fp32_.data()), input_img_length,
                        input_node_dims[0].data(), input_node_dims[0].size(), input_types[0], &input_tensors[0]));
        g_ort_->ReleaseMemoryInfo(memory_info);

        /* do inference */
        CheckStatus(g_ort_->Run(g_dmpr_s_.sess, nullptr, input_node_names.data(), (const OrtValue* const*)input_tensors.data(),
                        input_tensors.size(), output_node_names.data(), output_node_names.size(), output_tensors.data()));

        /* postprocess */
        assert (output_node_names.size() == 1);
        // fprintf(stderr, "retrieve output[0]: %s\n", output_node_names[0]);
        void* output_buffer;
        CheckStatus(g_ort_->GetTensorMutableData(output_tensors[0], &output_buffer));
        float* float_buffer = reinterpret_cast<float*>(output_buffer);
        int output_size = 1;

        // float_buffer[1*6*16*16]
        for(int k=0; k<output_node_dims[0].size(); k++){
            output_size *= output_node_dims[0][k];
            // fprintf(stderr, "output_node_dims[%d] - %d\n", k, output_node_dims[0][k]);
        }
        // for(int k=0; k<output_size; k++){
        //     fprintf(stderr, "%f\n", float_buffer[k]);
        // }

        /* retrieve marking-points number */
        const size_t mp_num = output_node_dims[0][2] * output_node_dims[0][3];
        /* 
        // float_buffer[1*6*16*16]:
            0-score
            1-shape
            ...
            5-sin
        */
        result.score.assign(float_buffer, float_buffer+mp_num);
        result.shape.assign(float_buffer+mp_num, float_buffer+mp_num*2);
        result.x.assign(float_buffer+mp_num*2, float_buffer+mp_num*3);
        result.y.assign(float_buffer+mp_num*3, float_buffer+mp_num*4);
        result.cos.assign(float_buffer+mp_num*4, float_buffer+mp_num*5);
        result.sin.assign(float_buffer+mp_num*5, float_buffer+mp_num*6);
        return true;
    }

    void test_dmpr_model(){
        cv::Mat img = cv::Mat::ones(640, 640, CV_8UC3);
        run_dmpr_model(img, g_det_);
    }
...
private:
    ...
    void destroy_ort(){
        ...
        if(g_dmpr_s_.sess) g_ort_->ReleaseSession(g_dmpr_s_.sess);
